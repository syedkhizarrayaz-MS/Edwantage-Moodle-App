...
export default class CoreLoginSitePage implements OnInit {
    ...
    async ngOnInit(): Promise<void> {
        ...
        if (sites.length) {
            key = await this.initSiteSelector();
        } else {
            const url = await this.consumeInstallReferrerUrl() ?? '';
            const showOnboarding = CoreConstants.CONFIG.enableonboarding && !CorePlatform.isIOS();
        ...
        this.siteForm = this.formBuilder.group({
            siteUrl: [key, this.customerKeyValidator()],
        });
    ...
    // key mapper
    private customerKeyValidator(): ValidatorFn {
        return (control: AbstractControl): ValidationErrors | null => {
            const value = control.value.trim();
            const valid = value.length >= 20 && /^[A-Z0-9]+$/.test(value);

            return valid ? null : { invalidKey: true };
        };
    }
    // get url

    private async getUrlFromKey(key: string): Promise<string> {

        const apiUrl = 'https://dev.meteoricsolutions.com/KeyUrlMapBackend/geturl.php';

        try {
            const response = await firstValueFrom(
                this.http.get<{ success: boolean; url?: string; message?: string }>(
                    apiUrl,
            ...
            throw new CoreError(response.message || 'core.login.invalidcustomerkey');
        } catch (error) {
            if (error instanceof CoreError) {
                throw error;
            }
            throw new CoreError('core.networkerrormsg');
        }
    }
     ...
     *
     * @param key The URL to connect to.
    ...
    //     }
    // }
    async connect(key: string, e?: Event): Promise<void> {
        e?.preventDefault();
        e?.stopPropagation();
        ...
        try {
            const url = await this.getUrlFromKey(key.trim());
    ...
    protected async showLoginIssue(url: string, error: CoreError): Promise<void> {
        ...
        if (error.message === 'core.login.invalidcustomerkey') {
            errorMessage = Translate.instant('core.login.invalidcustomerkey');
        } else if (error.message === 'core.networkerrormsg') {
            errorMessage = Translate.instant('core.networkerrormsg');
        }
